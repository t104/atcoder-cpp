...
struct rectangle {
    ...
    bool is_inside(point p) {
        return in(p.x, top_left.x, top_left.x + width + 1) &&
               in(p.y, top_left.y, top_left.y + height + 1);
    }
...
struct board {
    ...
    board(vector<rectangle> ads, vector<int> reqs) : ads(ads), reqs(reqs) {
        score_all();
    }
    ...
    void update_if_better(int x, int direction, int length) {
        rectangle new_rect = ads[x].extend(direction, length);
...
struct solver {
    ...
    void solve() {
        ...
        while((chrono::system_clock::now() - start).count() < LIMIT) {
            ...
            bd.update_if_better(rect, direction, 1);
        }
    }
